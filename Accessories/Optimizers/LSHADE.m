...
[monitor_flag, evaluated, D, HM, X, valParents, lb, ub, objfunc, currentNP, initNP, minNP, maxFEs, fbest] = parseARGS(varargin);
...
while evaluated < maxFEs
    ...
    if best < fbest
        ...
        activity = [activity, 1];
        if monitor_flag
            fprintf('bestfit: %.2e (%.0f%%, %d / %d)\n', fbest, 100 * evaluated / maxFEs,evaluated, maxFEs);
        end
    else
        activity = [activity, 0];
    end
...
function [monitor_flag, evaluated, D, HM, X, valParents, lb, ub, objfunc, currentNP, initNP, minNP, maxFEs, fbest] = parseARGS(varargin)
varargin = varargin{:};
initPop = [];
initFit = [];
maxGen = [];
...
objfunc = [];
monitor_flag = false;
proStr = {'monitor_flag', 'lb', 'ub', 'objfunc', 'initPop', 'initFit', 'xbest', 'fbest', 'HM', 'maxGen', 'NP', 'initNP', 'meanNP', 'maxFEs', 'dims', 'minNP'};
